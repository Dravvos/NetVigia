...
namespace NetVigia.BLL.Repository
{
    public class ServerRepository : IServerRepository
    {
        ...
        {
            var model = await con.Servers.Where(x => x.Active).Include(x => x.HTTPMethod).Include(x => x.MonitoringType).ToListAsync();
            return Map<List<ServerDTO>>.Convert(model);
        }

        public async Task<List<ServerDTO>> GetAllAsync(Guid userId)
        {
            var model = await con.Servers.Where(x => x.UserId == userId).Include(x => x.HTTPMethod).Include(x => x.MonitoringType).ToListAsync();
            return Map<List<ServerDTO>>.Convert(model);
        }

        public async Task<ServerDTO> GetByIdAsync(Guid id)
        {
            var model = await con.Servers.Include(x => x.MonitoringType).Include(x => x.HTTPMethod).FirstOrDefaultAsync(x => x.Id == id);
            return Map<ServerDTO>.Convert(model);
        }

        public async Task<List<ServerDTO>> GetByNomeAsync(Guid userId, string nome)
        {
            var model = await con.Servers.Where(x => x.UserId == userId && x.URL.ToLower().Contains(nome.ToLower())).Include(x => x.HTTPMethod)
                .Include(x => x.MonitoringType).ToListAsync();
        ...
        {
            var model = await con.Servers.Include(x => x.MonitoringType).Include(x => x.HTTPMethod).FirstOrDefaultAsync(x => x.URL == url);
            return Map<ServerDTO>.Convert(model);
        }
